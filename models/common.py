...
class Conv(nn.Module):
    ...
    def forward_fuse(self, x): # fuse bn and conv
...
class RepNBottleneck(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, skip connection, groups, kernels, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = RepConvN(c1, c_, k[0], 1)
        self.cv2 = Conv(c_, c2, k[1], 1, g=g)
...
class RepNCSPELAN4(nn.Module):
    ...
    def forward_split(self, x): # inference: for m.forward = m.forward_split
        y = list(self.cv1(x).split((self.c, self.c), 1))
        y.extend(m(y[-1]) for m in [self.cv2, self.cv3])
        return self.cv4(torch.cat(y, 1))
